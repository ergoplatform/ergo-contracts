...
trait SigmaContract {
  ...
  @NeverInline
  @Reified("T") @ignore
  def Collection[T](items: T*)(implicit cT: RType[T]): Coll[T] =
    ??? //this.builder.Colls.fromItems[T](items:_*)
  ...
  def atLeast(bound: Int, props: Coll[SigmaProp]): SigmaProp =
    this.builder.atLeast(bound, props)
  ...
  implicit def booleanToSigmaProp(source: Boolean): SigmaProp =
    this.builder.sigmaProp(source)
  ...
  @Reified("T")
  def substConstants[T](
    scriptBytes: Coll[Byte],
    positions: Coll[Int],
    newValues: Coll[T]
  )(implicit cT: RType[T]): Coll[Byte] =
    this.builder.substConstants(scriptBytes, positions, newValues)
