    ...
    while (i < len) {
      ...
      if (b.contains(kv._1))
        throw new IllegalArgumentException(
          s"Cannot transform collection $this to Map: duplicate key in entry $kv"
        )
      b = b + kv
      i += 1
    }
...
//  }
  ...
  def getOrElse(i: Int, default: A): A =
    if (i >= 0 && i < toArray.length) toArray(i) else default
...
//  }
  ...
  def foldLeft[B](zero: B, op: ((B, A)) => B): B =
    ??? //toArray.foldLeft(zero)((b, a) => op((b, a)))

  def slice(from: Int, until: Int): Coll[A] =
    ??? //builder.fromArray(toArray.slice(from, until))
...
//  }
  ...
  override def indexWhere(p: A => Boolean, from: Int): Int =
    ??? // toArray.indexWhere(p, from)

  override def lastIndexWhere(p: A => Boolean, end: Int): Int =
    ??? // toArray.lastIndexWhere(p, end)
...
//  }
  ...
  override def mapReduce[K: RType, V: RType](
    m: A => (K, V),
    r: ((V, V)) => V
  ): Coll[(K, V)] = ???
//  {
