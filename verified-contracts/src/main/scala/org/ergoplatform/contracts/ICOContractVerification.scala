...
sealed abstract class ICOContract extends SigmaContract {
  ...
  def ICOIssuanceContract(
    ...
    val correctTokensIssued = OUTPUTS(0).tokens.size == 1 && SELF.value == OUTPUTS(0)
          .tokens(0)
          ._2
...
case object ICOContractVerification extends ICOContract {
  ...
  def proveICOFundingContractForHeightAbove2000(
    ctx: Context,
    nextStageScriptHash: Coll[Byte],
    feeBytes: Coll[Byte]
  ): Boolean = {
  ...
  def failICOFundingSelfIndexNotZero(
    ctx: Context,
    nextStageScriptHash: Coll[Byte],
    feeBytes: Coll[Byte]
  ): Boolean = {
