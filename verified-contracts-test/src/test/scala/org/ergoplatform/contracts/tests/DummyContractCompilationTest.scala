...
/*
  ...
  property("dummy contract2 ergo tree") {
    forAll(unsignedIntGen) { l =>
      ...
      val expectedProp =
        BoolToSigmaProp(BinAnd(GE(Height, IntConstant(s)), LE(Height, LongConstant(e))))
      assert(c.prop == expectedProp)
    }
  }
  ...
  property("dummy contract2 scalaFunc") {
    ...
    val contractFalse = DummyContractCompilation.contract2Instance(
  ...
  property("dummy contract3 scalaFunc") {
    ...
    val contractFalse =
      DummyContractCompilation.contract3Instance(emptyByteColl, emptyLongColl)
    forAll(ergoLikeContextGen.map(_.toSigmaContext(IR, isCost = false, Map()))) { ctx =>
      assert(contractTrue.scalaFunc(ctx).isValid)
      assert(!contractFalse.scalaFunc(ctx).isValid)
    }
  ...
  property("dummy contract4") {
    forAll(ergoLikeContextGen.map(_.toSigmaContext(IR, false)), proveDlogGen) {
      case (ctx, proveDlog) =>
        ...
        val expectedProp = SigmaAnd(
          BoolToSigmaProp(GE(Height, IntConstant(0))),
          SigmaPropConstant(proveDlog)
        )
        assert(c.prop == expectedProp)
  ...
  property("dummy contract5") {
    forAll(
      ergoLikeContextGen.map(_.toSigmaContext(IR, false)),
      proveDlogGen,
      proveDlogGen
    ) {
      case (ctx, proveDlog1, proveDlog2) =>
        ...
        val expectedProp =
          SigmaAnd(SigmaPropConstant(proveDlog1), SigmaPropConstant(proveDlog2))
        assert(c.prop == expectedProp)
  ...
  property("dummy contract6 ergo tree") {
    forAll(collOfGen(byteCollGen(0, 10), 10)) { cc =>
      ...
      val expectedProp = BoolToSigmaProp(
        GT(
          SizeOf(
            ByIndex(collTree, IntConstant(0)).asInstanceOf[Value[SCollection[SType]]]
          ),
          IntConstant(0)
        )
      )
  ...
  property("dummy contract6 scalaFunc") {
    ...
    val firstItemEmpty = cc
      .append(
        new CollOverArray[Coll[Byte]](
          Array[Coll[Byte]](new CollOverArray[Byte](Array.empty[Byte]))
        )
      )
      .reverse
  ...
  property("dummy contract7 ergo tree") {
    forAll(collOfGen(byteCollGen(0, 10).map((_, arbLong.arbitrary.sample.get)), 10)) {
      tc =>
