...
class DexSellOrderPartialMatchTest
  ...
  property("sell order, sorting of counter orders") {
    val verifier = new ErgoLikeTestInterpreter
    val buyerPk =
      (new ContextEnrichingTestProvingInterpreter).dlogSecrets.head.publicImage
    val sellerPk =
    ...
    val returnBox = ErgoBox(
      value               = tokenAmount * sellerTokenPrice,
      ergoTree            = sellerPk,
      creationHeight      = 0,
      additionalRegisters = Map(ErgoBox.R4 -> ByteArrayConstant(sellOrderBox.id))
    )

    val buyOrderBox1 = ErgoBox(
      value = tokenAmount * (buyer1TokenPrice + dexFeePerToken),
      ergoTree = DexLimitOrderContracts
        .buyerContractInstance(
          DexBuyerContractParameters(buyerPk, tokenId, buyer1TokenPrice, dexFeePerToken)
        )
        .ergoTree,
      creationHeight = 0,
      additionalRegisters = Map(
        ErgoBox.R4 -> ByteArrayConstant(tokenId),
        ErgoBox.R5 -> LongConstant(buyer1TokenPrice),
        ErgoBox.R6 -> LongConstant(dexFeePerToken)
      )
    )

    val buyOrderBox2 = ErgoBox(
      value = tokenAmount * (buyer2TokenPrice + dexFeePerToken),
      ergoTree = DexLimitOrderContracts
        .buyerContractInstance(
          DexBuyerContractParameters(buyerPk, tokenId, buyer2TokenPrice, dexFeePerToken)
        )
        .ergoTree,
      creationHeight = 0,
      additionalRegisters = Map(
        ErgoBox.R4 -> ByteArrayConstant(tokenId),
        ErgoBox.R5 -> LongConstant(buyer2TokenPrice),
        ErgoBox.R6 -> LongConstant(dexFeePerToken)
      )
    )
    ...
    verifySpendingBoxes(IndexedSeq(buyOrderBox1, buyOrderBox2, sellOrderBox)) shouldBe true
    verifySpendingBoxes(IndexedSeq(buyOrderBox2, buyOrderBox1, sellOrderBox)) shouldBe false
  }
}
