...
class DexBuyOrderPartialMatchTest
  ...
  property(
    "buy order partial matching, exploit by counter order token price manipulation"
  ) {
    val verifier = new ErgoLikeTestInterpreter
    val buyerPk =
      (new ContextEnrichingTestProvingInterpreter).dlogSecrets.head.publicImage
    ...
    def ctxWithCounterOrderTokenPrice(maliceTokenPrice: Long) = {
      ...
      val sellOrderBox = ErgoBox(
        ...
        additionalRegisters = Map(
          ErgoBox.R4 -> ByteArrayConstant(tokenId),
          ErgoBox.R5 -> LongConstant(maliceTokenPrice)
        )
      )
    ...
    // should trigger overflow on spreadPerToken calculation
    verifier
      .verify(
        buyOrderBox.ergoTree,
        ctxWithCounterOrderTokenPrice(-Long.MinValue),
        ProverResult.empty,
        fakeMessage
      )
      .isFailure shouldBe true

    // should trigger overflow on fullSpread calculation
    verifier
      .verify(
        buyOrderBox.ergoTree,
        ctxWithCounterOrderTokenPrice(-Long.MinValue + tokenPrice),
        ProverResult.empty,
        fakeMessage
      )
      .isFailure shouldBe true

    // sell order token price greater than buy order should result in boxesAreSortedBySpread fail
    verifier
      .verify(
        buyOrderBox.ergoTree,
        ctxWithCounterOrderTokenPrice(tokenPrice + 1),
        ProverResult.empty,
        fakeMessage
      )
      .get
    ...
    // to check that other env does not stop buyer box from spending
    verifier
      .verify(
        buyOrderBox.ergoTree,
        ctxWithCounterOrderTokenPrice(tokenPrice),
        ProverResult.empty,
        fakeMessage
      )
      .get
